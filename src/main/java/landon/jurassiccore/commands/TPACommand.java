...
public class TPACommand implements CommandExecutor {
  ...
  public TPACommand(JurassicCore instance) {
  ...
  public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
    ...
    if (args.length != 1) {
      player.sendMessage(
          ChatColor.translateAlternateColorCodes('&', configLoad.getString("Commands.TPA.Invalid.Message")));
    ...
    if (targetPlayer == null) {
      player.sendMessage(
          ChatColor.translateAlternateColorCodes('&', configLoad.getString("Commands.TPA.Offline.Message")));
      player.playSound(player.getLocation(), Sound.ANVIL_LAND, 1.0F, 1.0F);
      return true;
    } 
    if (targetPlayer.getName().equals(player.getName())) {
      player.sendMessage(
          ChatColor.translateAlternateColorCodes('&', configLoad.getString("Commands.TPA.Yourself.Message")));
      player.playSound(player.getLocation(), Sound.ANVIL_LAND, 1.0F, 1.0F);
      return true;
    } 
    ...
    if (pendingExpiry != null && pendingExpiry.getType() == ExpiryType.Teleport) {
      player.sendMessage(ChatColor.translateAlternateColorCodes('&', 
            configLoad.getString("Commands.TPA.Teleporting.Player.Message")));
      player.playSound(player.getLocation(), Sound.ANVIL_LAND, 1.0F, 1.0F);
      return true;
    } 
    if (playerData.getTimeout(TimeoutType.Teleport).getTime() != 0L) {
      player.sendMessage(ChatColor.translateAlternateColorCodes('&', 
            configLoad.getString("Commands.TPA.Request.Player.Message")));
      player.playSound(player.getLocation(), Sound.ANVIL_LAND, 1.0F, 1.0F);
      return true;
    } 
    ...
    if (targetPlayerData.getTimeout(TimeoutType.Teleport).getTime() != 0L) {
      player.sendMessage(ChatColor.translateAlternateColorCodes('&', 
            configLoad.getString("Commands.TPA.Request.Target.Message")));
      player.playSound(player.getLocation(), Sound.ANVIL_LAND, 1.0F, 1.0F);
      return true;
    } 
    playerData.setTeleport(targetPlayer.getUniqueId());
    playerData.setTeleportToPlayer(true);
    playerData.getTimeout(TimeoutType.Teleport).setTime(System.currentTimeMillis() + 30000L);
    player.sendMessage(ChatColor.translateAlternateColorCodes('&', 
          configLoad.getString("Commands.TPA.Notify.Player.Message").replace("%player", targetPlayer.getName())));
    player.playSound(player.getLocation(), Sound.VILLAGER_YES, 1.0F, 1.0F);
    targetPlayer.sendMessage(ChatColor.translateAlternateColorCodes('&', 
          configLoad.getString("Commands.TPA.Notify.Receiver.Message").replace("%player", player.getName())));
    return true;
  }
}
