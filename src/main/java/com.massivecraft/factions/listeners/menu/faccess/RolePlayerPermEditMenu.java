...
public class RolePlayerPermEditMenu
        extends GUIMenu {
    ...
    public RolePlayerPermEditMenu(Player player, FPlayer fplayer, FPlayer editting, Faction faction) {
        super("Player Role Permissions", RolePlayerPermEditMenu.fitSlots(RolePerm.values().length + 1));
        this.player = player;
    ...
    public void drawItems() {
        int index = 0;
        Set<RolePerm> permissions = this.faction.getRolePerms(this.editting, false);
        for (RolePerm perm : RolePerm.values()) {
            ...
            this.setItem(index++, new ClickableItemStack(new CCItemBuilder(Material.STAINED_GLASS_PANE, (allowed ? CC.GreenB : CC.RedB) + perm.getDisplay()).lore(lore).data((short) (allowed ? DyeColor.LIME : DyeColor.RED).getWoolData()).build()).setClickCallback(e -> {
                if (!(this.faction.attached() && this.fplayer.hasFaction() && this.editting.getFaction().equals(this.faction))) {
                ...
                if (this.fplayer.getRole() == Role.COLEADER && this.editting.getRole() == Role.COLEADER && !this.fplayer.isAdminBypassing()) {
                    this.player.sendMessage(CC.RedB + "(!) " + CC.Red + "Co-Leaders do not have access to edit other Co-Leaders roles!");
                    this.player.closeInventory();
                    return;
                }
                boolean nowAllowed = !allowed;
                this.faction.setPlayerRolePermission(this.editting, permissions, perm, nowAllowed);
                this.faction.sendMessage(CC.YellowB + "(!) " + CC.Yellow + this.player.getName() + " has " + (nowAllowed ? CC.GreenB + "GRANTED" : CC.RedB + "REMOVED") + CC.Yellow + " the role permission for '" + CC.YellowU + perm.getDisplay() + CC.Yellow + "' " + (nowAllowed ? "to" : "from") + " " + this.editting.getNameAsync() + CC.Yellow + "!");
                P.p.getFlogManager().log(this.faction, FLogType.ROLE_PERM_EDIT, this.player.getName(), (nowAllowed ? CC.GreenB + "GRANTED" : CC.RedB + "REMOVED") + " " + CC.YellowU + perm.getDisplay(), (nowAllowed ? "to" : "from") + " " + this.editting.getNameAsync());
                this.player.playSound(this.player.getLocation(), Sound.LEVEL_UP, 1.0f, 1.1f);
                this.drawItems();
            }));
        }
        this.setItem(this.getSize() - 1, new ClickableItemStack(new CCItemBuilder(Material.ARROW, CC.RedB + "Previous Menu", new String[]{CC.Gray + "Click to return to player role menu."}).build()).setClickCallback(e -> this.getPreviousMenu().open(this.player)));
    }
}
